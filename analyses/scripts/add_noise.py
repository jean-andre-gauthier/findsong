...
def main():
  ...
  parser.add_argument("--inputfilesglob",
    help="clips to which to add noise (glob)", required=True, type=str)
  parser.add_argument("--noisefilepath", help="noise file to use (path)",
      required=True, type=str)
  parser.add_argument("--noiseweight", help="weight of the noise (float)",
    required=True, type=float)
  parser.add_argument("--outputfolderpath",
    help="folder where to save the processed clips (path)", required=True,
    type=str)
  ...
  if noiseWeight < 0.0:
    print(f"Error: noise weight must be larger or equal to 0.0 but is {noiseWeight}")
    exit(1)
