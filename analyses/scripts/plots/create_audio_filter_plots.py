...
def create_audio_filter_plot(audio_filter_plot_path):
    ...
    plt.suptitle("Matcher peformance with distorted audio", fontsize=12, y=0.05)
    plt.tight_layout(pad=4.0, w_pad=4.0, h_pad=4.0)

    indices = np.arange(1, 8)
    labels = np.array(["aecho", "aphaser", "chorus", "clean", "flanger", "highpass", "lowpass"])
    values = np.array([97.55, 97.91, 98.05, 99.36, 97.81, 97.88, 99.21])
    aecho, aphaser, chorus, clean, flanger, highpass, lowpass = axes[0].bar(indices, values)
    axes[0].set_xticks(indices)
    axes[0].set_xticklabels(labels, rotation=45)
    axes[0].set_ylim([95, 100])
    axes[0].set_ylabel("Recognition rate in %")

    cell_text = np.array([["aecho", "0.8:0.8:1000:0.8"], ["aphaser", "delay=5.0:speed=2.0"], ["chorus", "0.7:0.9:55:0.4:0.25:2"], ["clean", "-"], ["flanger", "delay=20:depth=5:regen=10:speed=2"], ["highpass", "f=440"], ["lowpass","f=440"]])
...
def create_pitch_plot(pitch_plot_path):
    ...
    create_plot(xs, "Pitch shift (halftones)", ys1, "Recognition rate in %", ys2, "Average match score", "Matcher performance with pitch shift", pitch_plot_path)
...
def create_tempo_plot(tempo_plot_path):
    ...
    create_plot(xs, "Tempo increase (percent)", ys1, "Recognition rate in %", ys2, "Average match score", "Matcher performance with tempo increase", tempo_plot_path)

def create_plot(xs, xs_label, ys1, ys_label1, ys2, ys_label2, title, file_name):
