...
class MatcherSpec extends FunSpec with Matchers with PrivateMethodTester {
  ...
  describe("peakPairsToSongOffsets") {
    val peakPairsToSongOffsets =
      PrivateMethod[SongOffsets]('peakPairsToSongOffsets)
    ...
    describe("when the song index is empty") {
      ...
      describe("when the signal doesn't contain any peak pair") {
        it("should return an empty song offset map") {
          ...
          val songOffsets = Matcher invokePrivate peakPairsToSongOffsets(
            peakPairs,
            songIndex)
    ...
    describe("when the song index contains exactly one peak pair") {
      val songIndex = SongIndex() + (SongIndexKey(1, 0, 1) -> Seq(
        SongIndexValue(1, song1)))
      ...
      describe(
        "when the signal contains a peak pair with a wrong first frequency") {
        it("should return an empty song offset map") {
          val peakPairs = Seq(
      ...
      describe(
        "when the signal contains a peak pair with a wrong second frequency") {
        it("should return an empty song offset map") {
          val peakPairs = Seq(
    ...
    describe("when the song index contains multiple peak pairs") {
      val songIndex = SongIndex() + (
        ...
        SongIndexKey(2, 0, 1) -> Seq(SongIndexValue(1, song1),
                                     SongIndexValue(2, song2),
                                     SongIndexValue(5, song3)),
        SongIndexKey(2, 1, 3) -> Seq(SongIndexValue(4, song1),
                                     SongIndexValue(6, song2)),
        SongIndexKey(2, 3, 1) -> Seq(SongIndexValue(10, song1)),
        SongIndexKey(4, 5, 3) -> Seq(SongIndexValue(0, song1),
                                     SongIndexValue(1, song2),
                                     SongIndexValue(2, song3),
                                     SongIndexValue(5, song4)),
  ...
  describe("songConfidenceToMatches") {
    val songConfidenceToMatches =
      PrivateMethod[Matches]('songConfidenceToMatches)

    describe("when the confidence map is empty") {
      it("should return an empty match list") {
        val songConfidence = SongConfidence()
        val matches = Matcher invokePrivate songConfidenceToMatches(
          songConfidence,
          s)
  ...
  describe("songOffsetsToSongConfidence") {
    val songOffsetsToSongConfidence =
      PrivateMethod[SongConfidence]('songOffsetsToSongConfidence)

    describe("when the offset map is empty") {
      it("should return an empty confidence map") {
        val songOffsets = SongOffsets()
        val songConfidence = Matcher invokePrivate songOffsetsToSongConfidence(
          songOffsets,
          s)
