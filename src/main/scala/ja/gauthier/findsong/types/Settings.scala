...
object Settings {
  ...
  case class CliArguments(
      ...
      peaksPerChunk: Int =
        ApplicationConfigArguments.ConstellationMap.peaksPerChunk,
      samplesPerCapture: Int =
        ApplicationConfigArguments.Recording.samplesPerCapture,
      samplesPerChunk: Int =
        ApplicationConfigArguments.Spectrogram.samplesPerChunk,
      samplesPerChunkStep: Int =
        ApplicationConfigArguments.Spectrogram.samplesPerChunkStep,
      sampleRate: Int = ApplicationConfigArguments.Preprocessing.sampleRate,
      scoreCoefficient: Int =
        ApplicationConfigArguments.Matching.scoreCoefficient,
  ...
  val argumentsParser = new scopt.OptionParser[CliArguments]("") {
    ...
    opt[Int]("fanout")
      ...
      .text(
        s"Maximal number of peaks that can be paired with any given peak (default = ${ApplicationConfigArguments.PeakPairs.fanout})")

    opt[Int]("greenLevel")
      .action((greenLevel, cliArguments) =>
        cliArguments.copy(greenLevel = greenLevel))
      .text(
        s"Threshold for a match score to be displayed in green (default = ${ApplicationConfigArguments.Matching.greenLevel})")
    ...
    opt[String]('m', "matcherGlob")
      ...
      .action((matcherGlob, cliArguments) =>
        cliArguments.copy(
          matcherGlob = Option(matcherGlob).filter(_.trim.nonEmpty)))
    ...
    opt[Int]("maxMatches")
      ...
      .text(
        s"Maximal number of matches returned by the search engine (default = ${ApplicationConfigArguments.Matching.maxMatches})")

    opt[Int]("peakDeltaF")
      .action((peakDeltaF, cliArguments) =>
        cliArguments.copy(peakDeltaF = peakDeltaF))
      .text(
        s"Frequency range in which a spectrogram cell has to be a local maximum to be considered a peak (default = ${ApplicationConfigArguments.ConstellationMap.peakDeltaF})")

    opt[Int]("peakDeltaT")
      .action((peakDeltaT, cliArguments) =>
        cliArguments.copy(peakDeltaT = peakDeltaT))
      .text(
        s"Time range in which a spectrogram cell has to be a local maximum to be considered a peak (default = ${ApplicationConfigArguments.ConstellationMap.peakDeltaF})")

    opt[Int]("peaksPerChunk")
      .action((peaksPerChunk, cliArguments) =>
        cliArguments.copy(peaksPerChunk = peaksPerChunk))
      .text(
        s"Maximal number of peaks in any given fingerprinting chunk (default = ${ApplicationConfigArguments.ConstellationMap.peaksPerChunk})")

    opt[Int]("samplesPerCapture")
      .action((samplesPerCapture, cliArguments) =>
        cliArguments.copy(samplesPerCapture = samplesPerCapture))
      .text(
        s"Size of a microphone recording in samples (default = ${ApplicationConfigArguments.Recording.samplesPerCapture})")

    opt[Int]("samplesPerChunk")
      .action((samplesPerChunk, cliArguments) =>
        cliArguments.copy(samplesPerChunk = samplesPerChunk))
      .text(
        s"Size of a fingerprinting chunk in samples (default = ${ApplicationConfigArguments.Spectrogram.samplesPerChunk})")
    ...
    opt[Int]("samplesPerChunkStep")
      ...
      .text(
        s"Size of the stride between two fingerprinting chunks in samples (default = ${ApplicationConfigArguments.Spectrogram.samplesPerChunkStep})")

    opt[Int]("sampleRate")
      .action((sampleRate, cliArguments) =>
        cliArguments.copy(sampleRate = sampleRate))
      .text(
        s"Fingerprinting / recording sample rate (default = ${ApplicationConfigArguments.Preprocessing.sampleRate})")

    opt[Int]("scoreCoefficient")
      .action((scoreCoefficient, cliArguments) =>
        cliArguments.copy(scoreCoefficient = scoreCoefficient))
      .text(
        s"Coefficient that is used in the match scoring function (default = ${ApplicationConfigArguments.Matching.scoreCoefficient})")

    opt[Int]("windowDeltaF")
      .action((windowDeltaF, cliArguments) =>
        cliArguments.copy(windowDeltaF = windowDeltaF))
      .text(
        s"Frequency range in which neighbouring peaks can be paired up (default = ${ApplicationConfigArguments.PeakPairs.windowDeltaF})")

    opt[Int]("windowDeltaT")
      .action((windowDeltaT, cliArguments) =>
        cliArguments.copy(windowDeltaT = windowDeltaT))
      .text(
        s"Time range in which neighbouring peaks can be paired up (default = ${ApplicationConfigArguments.PeakPairs.windowDeltaT})")

    opt[Int]("windowDeltaTi")
      .action((windowDeltaTi, cliArguments) =>
        cliArguments.copy(windowDeltaTi = windowDeltaTi))
      .text(
        s"Minimal time difference for neighbouring peaks to be paired up (default = ${ApplicationConfigArguments.PeakPairs.windowDeltaTi})")

    opt[Int]("yellowLevel")
      .action((yellowLevel, cliArguments) =>
        cliArguments.copy(yellowLevel = yellowLevel))
      .text(
        s"Threshold for a match score to be displayed in yellow (default = ${ApplicationConfigArguments.Matching.yellowLevel})")
  }
  ...
  def settings(args: Array[String]): Option[Settings] = {
    ...
    cliArgumentsOption.flatMap((cliArguments: CliArguments) =>
      Some(new Settings(cliArguments)))
  }
}
