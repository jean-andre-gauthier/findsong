...
object Indexer {
  ...
  /**
    ...
    */
  def indexSongs(implicit executionContext: ExecutionContext,
                 settings: Settings): Future[SongIndex] = {
    val songFiles = Glob
      .getMatchingFiles(settings.General.indexerGlob)
      .toIterable
      .to[scala.collection.immutable.Iterable]
    Future
      .foldLeft(songFiles.map((songFile: File) =>
        Future {
          ...
          songIndexEntries
      }))(Seq[SongIndex]())((songIndex, songIndexEntries) =>
        songIndex :+ songIndexEntries)
